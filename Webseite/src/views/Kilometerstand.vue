...
export default defineComponent({
  ...
  setup() {
    ...
    function refresh() {
      ...
      store.dispatch("getDataForKilometerstand").then(() => {
    ...
    function open(id: number): void {
      window.open(`http://localhost:8081/kilometerstand/${id}`);
    }

    const data = computed(() => {
      return store.state.kilometerstand;
    });

    const dataWithoutTime = computed(() => {
      const keys = Object.keys(store.state.kilometerstand).filter(
        value => value != "time"
      );
      const newObject: { [index: string]: any } = {};
      for (const value of keys) {
        newObject[value] = store.state.kilometerstand[value];
