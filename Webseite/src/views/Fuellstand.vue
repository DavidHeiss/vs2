...
export default defineComponent({
  ...
  setup() {
    ...
    function refresh() {
      ...
      store.dispatch("getDataForFuellstand").then(() => {
    ...
    function open(id: number): void {
      window.open(`http://localhost:8081/fuellstand/${id}`);
    }

    const data = computed(() => {
      return store.state.fuellstand;
    });

    const dataWithoutTime = computed(() => {
      const keys = Object.keys(store.state.fuellstand).filter(
        value => value != "time"
      );
      const newObject: { [index: string]: any } = {};
      for (const value of keys) {
        newObject[value] = store.state.fuellstand[value];
