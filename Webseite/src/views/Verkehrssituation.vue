...
export default defineComponent({
  ...
  setup() {
    ...
    function refresh() {
      ...
      store.dispatch("getDataForVerkehrssituation").then(() => {
    ...
    function open(id: number): void {
      window.open(`http://localhost:8081/verkehrssituation/${id}`);
    }

    const data = computed(() => {
      return store.state.verkehrssituation;
    });

    const dataWithoutTime = computed(() => {
      const keys = Object.keys(store.state.verkehrssituation).filter(
        value => value != "time"
      );
      const newObject: { [index: string]: any } = {};
      for (const value of keys) {
        newObject[value] = store.state.verkehrssituation[value];
